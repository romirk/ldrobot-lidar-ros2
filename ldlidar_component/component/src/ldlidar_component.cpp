...
namespace ldlidar {
    LdLidarComponent::LdLidarComponent(const rclcpp::NodeOptions& options)
        : LifecycleNode("lidar_node", "", options), mDiagUpdater(this) {
    ...
    LdLidarComponent::~LdLidarComponent() = default;
    ...
    void LdLidarComponent::getDebugParams() {
        ...
        if (mDebugMode) {
            if (const rcutils_ret_t res = rcutils_logging_set_logger_level(
                get_logger().get_name(), RCUTILS_LOG_SEVERITY_DEBUG); res != RCUTILS_RET_OK) {
            ...
            if (const rcutils_ret_t res = rcutils_logging_set_logger_level(
                get_logger().get_name(), RCUTILS_LOG_SEVERITY_INFO); res != RCUTILS_RET_OK) {
    ...
    void LdLidarComponent::getLidarParams() {
        ...
        integer_range limits_int = {0, static_cast<int>(ROTATION::COUNTERCLOCKWISE), 1};
        getParam(
    ...
    void LdLidarComponent::lidarReadCallback(const char* byte, const size_t len) const {
    ...
    bool LdLidarComponent::initLidarComm() {
        ...
        if (!mUseDirectSerial) {
            ...
            for (auto [dev_name, data]: device_list) {
                RCLCPP_DEBUG_STREAM(get_logger(), "Device found: " << dev_name << "    " << data);
                if (strstr(data.c_str(), "CP2102")) {
                    mSerialPort = dev_name;
                }
            }
    ...
    void LdLidarComponent::lidarThreadFunc() {
        ...
        while (true) {
            ...
            if (const int nSub = count_subscribers(mScanTopic); nSub > 0) {
                mPublishing = true;
                if (mLidar->IsFrameReady()) {
                    rclcpp::Time ts = get_clock()->now();
                    const double dt = (ts - prev_ts).nanoseconds();
    ...
    void LdLidarComponent::callback_updateDiagnostic(diagnostic_updater::DiagnosticStatusWrapper& stat) {
        if (const rclcpp_lifecycle::State state = get_current_state(); state.id() == 3) {
            // ACTIVE
            stat.summary(
