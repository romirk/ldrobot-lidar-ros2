...
namespace ldlidar {
    ...
    LiPkg::LiPkg(
        rclcpp::Clock::SharedPtr clock, const UNITS unit, const ROTATION rotVerse,
            ...
            if (crc == pkg->crc8) {
                if (const double diff = (pkg->end_angle / 100 - pkg->start_angle / 100 + 360) % 360;
                    ...
                    const uint32_t angle_diff =
                            (static_cast<uint32_t>(pkg->end_angle) + 36000 -
                             static_cast<uint32_t>(pkg->start_angle)) %
                            36000;
                    ...
                    for (int i = 0; i < POINT_PER_PACK; i++) {
                        ...
                        mFrameTmp.push_back(
                            PointData(
                                point_data.angle, point_data.distance,
    ...
    bool LiPkg::AssemblePacket() {
        ...
        for (auto n: mFrameTmp) {
            ...
            if (n.angle - last_angle < (-350.f)) {
                ...
                std::sort(
                    tmp.begin(), tmp.end(), [](const PointData& a, const PointData& b) {
    ...
    void LiPkg::ToLaserscan(std::vector<PointData> src) {
        ...
        for (const auto point: src) {
            ...
            if (const int index =
                        static_cast<int>((angle - mOutScan->angle_min) / mOutScan->angle_increment);
