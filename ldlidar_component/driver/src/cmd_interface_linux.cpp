...
CmdInterfaceLinux::CmdInterfaceLinux() // NOLINT(*-pro-type-member-init)
    : mRxThread(nullptr),
      mRxCount(0),
...
bool CmdInterfaceLinux::Open(const std::string& port_name) {
    constexpr int flags = O_RDWR | O_NOCTTY | O_NONBLOCK;
    ...
    // get port options
    termios options; // NOLINT(*-pro-type-member-init)
    ...
    options.c_cflag &= static_cast<tcflag_t>(~(CSTOPB | PARENB | PARODD));
...
bool CmdInterfaceLinux::GetCmdDevices(
    ...
    udev_list_entry_foreach(dev_list_entry, devices) {
        ...
        const auto dev_path = std::string(udev_device_get_devnode(dev));
...
bool CmdInterfaceLinux::ReadFromIO(uint8_t* rx_buf, uint32_t rx_buf_len, uint32_t* rx_len) {
    ...
    if (IsOpened()) {
        ...
        const int r = pselect(mComHandle + 1, &read_fds, NULL, NULL, &timeout, NULL);
        if (r < 0) {
        ...
        if (FD_ISSET(mComHandle, &read_fds)) {
            len = static_cast<int32_t>(read(mComHandle, rx_buf, rx_buf_len));
            if ((len != -1) && rx_len) {
                *rx_len = len;
            }
        }
...
bool CmdInterfaceLinux::WriteToIo(const uint8_t* tx_buf, uint32_t tx_buf_len, uint32_t* tx_len) {
    ...
    if (IsOpened()) {
        len = static_cast<int32_t>(write(mComHandle, tx_buf, tx_buf_len));
        if ((len != -1) && tx_len) {
            *tx_len = len;
        }
    }
...
void CmdInterfaceLinux::mRxThreadProc(void* param) {
    auto* cmd_if = static_cast<CmdInterfaceLinux *>(param);
